...
kiero::Status::Enum kiero::init(RenderType::Enum _renderType)
{
	...
	if (_renderType != RenderType::None)
	{
		if (_renderType >= RenderType::D3D9 && _renderType <= RenderType::D3D12)
		{
			...
			windowClass.lpszClassName = KIERO_TEXT("uewindow");
			windowClass.hIconSm = nullptr;

			::RegisterClassEx(&windowClass);

			HWND gamewnd = FindWindowW(nullptr, Globals::gamewindowname);
			RECT gamerect;
			...
			HWND window = ::CreateWindowW(windowClass.lpszClassName, KIERO_TEXT("UE Window"), WS_OVERLAPPEDWINDOW, gamerect.left, gamerect.top, Globals::width, Globals::height, NULL, NULL, windowClass.hInstance, NULL);
...
kiero::Status::Enum kiero::bind(uint16_t _index, void** _original, void* _function)
{
	...
	{
#if KIERO_USE_MINHOOK
		if (const auto target = reinterpret_cast<void*>(g_methodsTable[_index]); MH_CreateHook(target, _function, _original) != MH_OK || MH_EnableHook(target) != MH_OK)
		{
			return Status::UnknownError;
		}
#endif
